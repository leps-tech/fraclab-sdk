...
def _write_test_bundle(bundle_dir: Path) -> None:
    ...
    seconds = np.arange(0, 600, dtype=float)
    ts_us = (seconds * 1_000_000.0).astype(np.int64)
    slurry_rate = np.where(seconds < 60, 0.0, 12.0)
    proppant_ratio = np.where(seconds < 120, 0.0, np.minimum((seconds - 120.0) * 0.08, 6.0))
    section_fluid_volume = np.cumsum(slurry_rate / 60.0)
    treating_pressure = 30.0 + 0.15 * slurry_rate ** 2 + 0.5 * proppant_ratio
...
def test_bh_prop_conc_algorithm_runs_end_to_end(tmp_path) -> None:
    workspace_src = Path("algorithms/bh-prop-conc/0.1.0")
    ...
    (input_dir / "params.json").write_text(
        json.dumps({"casingIdMm": 100.0, "wellboreDepthM": 1000.0}),
        encoding="utf-8",
    )
    ...
    assert summary["curveFamily"] == "bh_prop_conc_shift"
    assert summary["wellboreVolume"]["bbl"] > 0.0
    assert summary["lagSeconds"]["median"] >= 0.0
