...
- [10. Workbench 图形界面](#workbench-ui)
- [11. 完整示例](#complete-examples)
- [12. 附录 A: Bundle 结构详解](#appendix-a-bundle-structure)
...
Workbench UI 内置英文 / 简体中文双语支持，可在每个页面右上角切换语言；也可以通过环境变量 `FRACLAB_WORKBENCH_LANG=en` 或 `FRACLAB_WORKBENCH_LANG=zh-CN` 设定初始语言。
...
另外，**Python 标准库**也可以直接使用。

除上面这些以外，其他第三方包都**不应**作为算法依赖来写。即使某些包会随着 SDK 或 Workbench 一起安装，也不属于算法可依赖契约。
...
- 这里说的是**算法代码**的依赖约定，包括 `main.py` 和算法工作区里的 schema 定义代码。
- `fraclab-sdk[workbench]` 只是额外安装 Workbench UI 侧依赖：`streamlit>=1.30`、`streamlit-plotly-events>=0.0.6`、`pyarrow>=16.0.0`；这不等于算法运行时默认允许依赖它们。
- SDK 包自身的完整安装依赖，以 [pyproject.toml](/Users/kurthus/code/fraclab-sdk/pyproject.toml) 为准；不要把 SDK 内部实现依赖误读成算法白名单。
...
- 每个时间窗字段必须显式写 `unit="us"`；运行时固定按 `us` 数值处理，不接受秒/毫秒/日期时间，也不做单位换算。
...
- 若定义 `windowSlots` 与 `windowSlotFallbackNote`，图像上方会显示”下一时间窗”备注：
  - 还未选窗：显示第 1 条备注
  - 选完第 1 个窗后：显示第 2 条备注
...
```json
{
  "timeWindows_fracRecord_stage_5712": [
    {"itemKey": "00000", "min": 2291000000.0, "max": 2411000000.0},
    {"itemKey": "00004", "min": 7291000000.0, "max": 7411000000.0},
    {"itemKey": "00005", "min": 12291000000.0, "max": 12411000000.0}
  ]
}
```
...
- **`min`/`max`**: 时间窗边界，**统一为 `us` 数值**。
- 若该 dataset 未启用时间窗（用户未选），值为 `null`。
- 空列表 `[]` 不是合法值（建议 `minItems >= 1`）。

#### 时间值语义与单位规范

**统一规则：时间窗 `min`/`max` 始终为 `us` 数值。**

| 来源 | 说明 |
|------|------|
| Workbench 选择器输出 | `min`/`max` 按原始 `us` 数值写入 `params.json` |
| 算法内部处理 | 必须与时间窗同单位（`us`）；禁止"时间列 ms、窗口 us"混用 |
| Parquet 原始时间列 | 选择器/预览只接受数值时间列，并统一按 `us` 解释 |

**Workbench 时间列识别优先级（大小写不敏感）：**

`ts_us` > `timestamp_us` > `time_us` > `bucket_us` > `t_us` > `timestamp` > `bucket` > `ts` > `time` > `t`

Workbench 会从上述候选列中选一个作为 x 轴，并按以下规则处理：
- 只接受**数值**时间列
- 原始数值直接视为 `us`
- 不再推断 `ns/us/ms/s`
- 不再把数值列解释为 UTC 日期时间
...
**算法侧强制要求：**
1. 算法读取数据后，必须将时间列保持/转换为 `us` 后再与窗口 `min`/`max` 做比较
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `FRACLAB_WORKBENCH_LANG` | `zh-CN` | 初始 UI 语言（`en` 或 `zh-CN`） |
...
| 7. Schema Edit | 输入参数编辑 | 在线编辑 `schema/inputspec.py` 和 `schema/base.py` |
...
Parquet 预览会自动识别数值时间列（优先级：`ts_us` > `timestamp_us` > `time_us` > `bucket_us` > `t_us` > `timestamp` > `bucket` > `ts` > `time` > `t`），并统一按 `us` 数值显示，对大数据集进行降采样以保证显示性能。
